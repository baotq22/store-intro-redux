...
const RootComponent = (props) => {
    ...
    setCart((prevCart) => {
      const updatedProducts = prevCart.products.map((cartProduct) => {
        if (cartProduct.id === product.id) {
          return { ...cartProduct, qty: cartProduct.qty + 1, price: product.price * (cartProduct.qty + 1) };
        }
        return cartProduct;
      });
      const updatedTotalPrice = updatedProducts.reduce((acc, item) => acc + item.price, 0);

      return { products: updatedProducts, totalPrice: updatedTotalPrice };
    })
  }
  ...
  const removeProductFromCart = (product) => {
    setCart((prevCart) => {
      const updatedProducts = prevCart.products.map((cartProduct) => {
        if (cartProduct.id === product.id) {
          const newQty = Math.max(cartProduct.qty - 1, 0);
          return { ...cartProduct, qty: newQty, price: product.price * newQty };
        }
        return cartProduct;
      });
      const updatedTotalPrice = updatedProducts.reduce((acc, item) => acc + item.price, 0);
